  ...
  return (
    <div>
      {items?.map((item: any) => (
        <div
          key={item.id}
          className="w-1262pxr h-53pxr text-14pxr font-medium leading-21pxr text-text-sub flex items-center"
        >
          <div className="ml-18pxr w-79pxr">{item.code}</div>
          <div className="w-86pxr">{item.name}</div>
          <div className="w-99pxr">{item.isBoxCollect ? "박스" : "리픽백"}</div>
