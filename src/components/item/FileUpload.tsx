...
export default function FileUpload() {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    await Promise.all(
      acceptedFiles.map(async (file) => {
        const presignedUrlData = await getPresignedUrl(file.name, "IMAGE");
  ...
  return (
    <div
      {...getRootProps()}
      className={`w-390pxr rounded-10pxr border-gray-300 p-4 ${
        isDragActive ? "bg-dark-gray" : "bg-circle-gray"
      }`}
      style={{
        minHeight: "104px", // 기본 높이
        height:
          fileStatuses.length > 0
            ? `${40 + fileStatuses.length * 40}px`
            : "104px", // 업로드된 파일 수에 따라 동적 높이
      }}
    >
      <input {...getInputProps()} />
      {fileStatuses.length === 0 ? (
